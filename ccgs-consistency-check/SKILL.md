...
description: "Scan all GDDs against the entity registry to catch cross-document inconsistencies — same entity with different stats, same item with different values, same formula with different variables. Write-time drift safety net; run after each new GDD and before /create-architecture."
---

## Cowork Adaptation Notes (read first)

Converted from Claude Code Game Studios (github.com/Donchitos/Claude-Code-Game-Studios) for Claude Cowork. Apply these substitutions when following the instructions below:

1. **Agents**: Named agents (e.g. `game-designer`, `unity-specialist`) are not Cowork subagents. When told to "spawn an agent via Task", read the matching file in this skill's bundled `roles/` folder and either (a) launch a general-purpose subagent via the Agent tool with that role file's content prepended to the brief, or (b) adopt the role inline and continue. If a role file is not bundled, adopt the role inline from its name.
2. **Project paths**: `design/`, `docs/`, `production/`, `src/` live in the user's connected project folder. Create them there as needed. Bundled reference docs are in this skill's `refs/` and `templates/` folders.
3. **Slash commands**: `/skill-name` means "the ccgs-skill-name skill". Installed set: start, brainstorm, map-systems, design-system, consistency-check, create-architecture, create-epics, create-stories, story-readiness, sprint-plan, dev-story, code-review, story-done, propagate-design-change, reverse-document. If a referenced skill is NOT in this set, tell the user it isn't installed and offer to do the equivalent steps manually.
4. **Hooks**: Claude Code hooks do not run in Cowork. Do their checks manually: validate JSON files you write, follow naming conventions, no hardcoded magic numbers, and update `production/session-state/active.md` at the start and end of each work session.
5. **Engine**: This project targets **Unity (C#)**. Ignore Godot and Unreal routing rows.
6. **Review mode**: If `production/review-mode.txt` doesn't exist in the project folder, default to `lean`.
